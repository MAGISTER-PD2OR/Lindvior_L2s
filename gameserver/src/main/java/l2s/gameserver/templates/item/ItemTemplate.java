...
public abstract class ItemTemplate extends StatTemplate
{
	...
	public void init()
	{
		int ordinal0C = Integer.decode("0x0C");
		int ordinal80 = Integer.decode("0x80");
		if(isWeapon())
		{
			attachFunc(new FuncTemplate(null, "Enchant", Stats.POWER_ATTACK, ordinal0C, 0));
			attachFunc(new FuncTemplate(null, "Enchant", Stats.MAGIC_ATTACK, ordinal0C, 0));
		}
		else if(isArmor())
		{
			if(_grade.extOrdinal() >= ItemGrade.R.extOrdinal())
			{
				...
				{
					attachFunc(new FuncTemplate(null, "EnchantRGrade", Stats.P_EVASION_RATE, ordinal80, 0));
					attachFunc(new FuncTemplate(null, "EnchantRGrade", Stats.M_EVASION_RATE, ordinal80, 0));
				}
				else if(_exType == ExItemType.UPPER_PIECE || _exType == ExItemType.FULL_BODY)
				{
					attachFunc(new FuncTemplate(null, "EnchantRGrade", Stats.POWER_ATTACK, ordinal80, 0));
					attachFunc(new FuncTemplate(null, "EnchantRGrade", Stats.MAGIC_ATTACK, ordinal80, 0));
				}
				else if(_exType == ExItemType.LOWER_PIECE)
				{
					attachFunc(new FuncTemplate(null, "EnchantRGrade", Stats.P_CRITICAL_DAMAGE, ordinal80, 0));
					//attachFunc(new FuncTemplate(null, "EnchantRGrade", Stats.M_CRITICAL_DAMAGE, ordinal80, 0)); TODO: [Bonux] Раскоминтить после правильной реализации формулы Маг. Крита 
				}
				else if(_exType == ExItemType.GLOVES)
				{
					attachFunc(new FuncTemplate(null, "EnchantRGrade", Stats.P_ACCURACY_COMBAT, ordinal80, 0));
					attachFunc(new FuncTemplate(null, "EnchantRGrade", Stats.M_ACCURACY_COMBAT, ordinal80, 0));
				}
				else if(_exType == ExItemType.FEET)
					attachFunc(new FuncTemplate(null, "EnchantRGrade", Stats.RUN_SPEED, ordinal80, 0));
			}

			if(_exType == ExItemType.SHIELD)
				attachFunc(new FuncTemplate(null, "Enchant", Stats.SHIELD_DEFENCE, ordinal0C, 0));
			else
				attachFunc(new FuncTemplate(null, "Enchant", Stats.POWER_DEFENCE, ordinal0C, 0));
			attachFunc(new FuncTemplate(null, "Enchant", Stats.MAX_HP, ordinal80, 0));
		}
		else if(isAccessory())
		{
			attachFunc(new FuncTemplate(null, "Enchant", Stats.MAGIC_DEFENCE, ordinal0C, 0));
		}
	}
			...
			if(!condition.test(env))
			{
				if(playable.isPlayer() && condition.getSystemMsg() != null)
				{
					if(condition.getSystemMsg().size() > 0)
						playable.sendPacket(new SystemMessage2(condition.getSystemMsg()).addItemName(getItemId()));
					else
