...
public final class ItemFunctions
{
	...
	public final static IStaticPacket checkIfCanEquip(Player player, ItemInstance item)
	{
		...
		if(!player.getClassId().isOfLevel(ClassLevel.AWAKED) && item.getGrade() != ItemGrade.R && item.getGrade() != ItemGrade.R95 && item.getGrade() != ItemGrade.R99)
		{
		...
		if(item.getTemplate().isCloak())
		{
			...
			if(item.getName().contains("Knight") && (player.getPledgeRank().ordinal() < PledgeRank.KNIGHT.ordinal() || player.getCastle() == null))
				return SystemMsg.YOU_DO_NOT_MEET_THE_REQUIRED_CONDITION_TO_EQUIP_THAT_ITEM;
