...
public final class TradeHelper
{
	...
	public final static void purchaseItem(Player buyer, Player seller, TradeItem item)
	{
		...
		if(!item.getItem().isStackable())
		{
			...
			{
				seller.sendPacket(new SystemMessage2(SystemMsg.S2S3_HAS_BEEN_SOLD_TO_C1_AT_THE_PRICE_OF_S4_ADENA).addName(buyer).addInteger(item.getEnchantLevel()).addItemName(item.getItemId()).addLong(price));
				buyer.sendPacket(new SystemMessage2(SystemMsg.S2S3_HAS_BEEN_PURCHASED_FROM_C1_AT_THE_PRICE_OF_S4_ADENA).addName(seller).addInteger(item.getEnchantLevel()).addItemName(item.getItemId()).addLong(price));
			}
			else
			{
				seller.sendPacket(new SystemMessage2(SystemMsg.S2_IS_SOLD_TO_C1_FOR_THE_PRICE_OF_S3_ADENA).addName(buyer).addItemName(item.getItemId()).addLong(price));
				buyer.sendPacket(new SystemMessage2(SystemMsg.S2_HAS_BEEN_PURCHASED_FROM_C1_AT_THE_PRICE_OF_S3_ADENA).addName(seller).addItemName(item.getItemId()).addLong(price));
			}
		}
		else
		{
			seller.sendPacket(new SystemMessage2(SystemMsg.S2_S3_HAVE_BEEN_SOLD_TO_C1_FOR_S4_ADENA).addName(buyer).addItemName(item.getItemId()).addLong(item.getCount()).addLong(price));
			buyer.sendPacket(new SystemMessage2(SystemMsg.S3_S2_HAS_BEEN_PURCHASED_FROM_C1_FOR_S4_ADENA).addName(seller).addItemName(item.getItemId()).addLong(item.getCount()).addLong(price));
		}
