...
public class EffectHealHPCP extends Effect
{
	...
	public boolean onActionTime()
	{
		...
		if(getEffected() != getEffector())
			getEffected().sendPacket(new SystemMessage2(SystemMsg.S2_HP_HAS_BEEN_RESTORED_BY_C1).addName(getEffector()).addInteger(Math.round(addToHp)));
		else
			getEffected().sendPacket(new SystemMessage2(SystemMsg.S1_HP_HAS_BEEN_RESTORED).addInteger(Math.round(addToHp)));
		...
		else
		{
			...
			getEffected().sendPacket(new SystemMessage2(SystemMsg.S2_CP_HAS_BEEN_RESTORED_BY_C1).addLong((long) addToCp).addName(getEffector()));
			if(addToCp > 0)
