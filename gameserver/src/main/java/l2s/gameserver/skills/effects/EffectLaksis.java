				...
				if(cha.isPlayer())
				{
					...
					if (heal)
					{
						...
						if (target.getCurrentCp() < target.getMaxCp())
						{
							...
							target.sendPacket(new SystemMessage2(SystemMsg.S1_CP_HAS_BEEN_RESTORED).addInteger((long) powerCP));
						...
						else
						{
							...
							target.sendPacket(new SystemMessage2(SystemMsg.S1_HP_HAS_BEEN_RESTORED).addInteger((long) powerHP));
