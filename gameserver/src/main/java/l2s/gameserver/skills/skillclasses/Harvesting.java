	...
	public void useSkill(Creature activeChar, List<Creature> targets)
	{
		...
		for(Creature target : targets)
			if(target != null)
			{
				...
				player.sendPacket(new SystemMessage2(SystemMsg.C1_HARVESTED_S3_S2S).addName(player).addLong(item.count).addItemName(item.itemId));
				if(player.isInParty())
				{
					SystemMessage2 smsg = new SystemMessage2(SystemMsg.C1_HARVESTED_S3_S2S).addString(player.getName()).addLong(item.count).addItemName(item.itemId);
