			...
			if(target != null)
			{
				...
				if(addToHp > 0)
				{
					...
					if(target.isPlayer())
					{
						if(activeChar != target)
							target.sendPacket(new SystemMessage2(SystemMsg.S2_HP_HAS_BEEN_RESTORED_BY_C1).addName(activeChar).addInteger(Math.round(addToHp)));
						else
				...
				if(target.isPlayer())
				{
					...
					if(addToCp > 0)
					{
						...
						target.sendPacket(new SystemMessage2(SystemMsg.S1_CP_HAS_BEEN_RESTORED).addInteger(Math.round(addToCp)));
					}
				}
