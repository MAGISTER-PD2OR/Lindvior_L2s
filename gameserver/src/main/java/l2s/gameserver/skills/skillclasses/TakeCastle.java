			...
			if(target != null)
			{
				...
				if(!siegeEvents.isEmpty())
				{
					...
					for(CastleSiegeEvent event : targetEvents)
					{
						if(siegeEvents.contains(event))
						{
							event.broadcastTo(new SystemMessage2(SystemMsg.CLAN_S1_HAS_SUCCESSFULLY_ENGRAVED_THE_HOLY_ARTIFACT).addString(player.getClan().getName()), CastleSiegeEvent.ATTACKERS, CastleSiegeEvent.DEFENDERS);
