...
import l2s.gameserver.network.l2.s2c.Ex2ndPasswordAck;
import l2s.gameserver.network.l2.s2c.Ex2ndPasswordCheck;
import l2s.gameserver.network.l2.s2c.Ex2ndPasswordVerify;
	...
	public boolean checkPassword(String password, boolean skipAuth)
	{
		...
		if(!password.equals(_password))
		{
			...
			else
			{
				...
				_activeClient.close(new Ex2ndPasswordVerify(Ex2ndPasswordVerify.PASSWORD_BAN, Config.EX_SECOND_AUTH_MAX_ATTEMPTS));
				return false;
			}
		}
		if(!skipAuth)
