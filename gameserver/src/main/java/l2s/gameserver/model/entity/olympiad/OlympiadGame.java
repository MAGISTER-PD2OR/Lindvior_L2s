...
import l2s.gameserver.network.l2.s2c.ExReceiveOlympiad;
...
public class OlympiadGame
{
	...
	{
		ExReceiveOlympiad.MatchResult packet = new ExReceiveOlympiad.MatchResult(false, winnerMember.getName());
		...
		if(winnerPlayer != null)
		{
			...
			winnerPlayer.sendPacket(SystemMessage2.obtainItems(item.getItemId(), getType().getReward(), 0));
		...
		broadcastPacket(new SystemMessage2(SystemMsg.CONGRATULATIONS_C1_YOU_WIN_THE_MATCH).addString(winnerMember.getName()), false, true);

		//FIXME [VISTALL] нужно ли?
		//broadcastPacket(new SystemMessage2(SystemMsg.C1_HAS_EARNED_S2_POINTS_IN_THE_GRAND_OLYMPIAD_GAMES).addString(winnerMember.getName()).addInteger(pointDiff), true, false);
		//broadcastPacket(new SystemMessage2(SystemMsg.C1_HAS_LOST_S2_POINTS_IN_THE_GRAND_OLYMPIAD_GAMES).addString(looseMember.getName()).addInteger(pointDiff), true, false);
