...
public final class FightBattleArenaObject implements Serializable, Comparable<FightBattleArenaObject>
{
	...
	private void startBattle()
	{
		...
		for(FightBattlePlayerObject member : getMembers())
		{
			...
			broadcastPacket(new ExOlympiadUserInfo(player, member == getMember1() ? 1 : 2), true);
		}
	}
	...
	public void stopBattle()
	{
		...
		if(_winner != 0)
			broadcastPacket(new SystemMessage2(SystemMsg.CONGRATULATIONS_C1_YOU_WIN_THE_MATCH).addString(_winner == 1 ? getMember1().getName() : getMember2().getName()), true);
		else
		...
		//broadcastPacket(new ExOlympiadMode(0), false);
		broadcastPacket(ExOlympiadMatchEnd.STATIC, false);
	}
	...
	public void teleportPlayer(FightBattlePlayerObject member)
	{
		...
		player.sendPacket(new ExOlympiadMode(member == getMember1() ? 1 : 2));
	}
	...
	public void broadcastStatusUpdate(Player player)
	{
		...
		broadcastPacket(new ExOlympiadUserInfo(player, member == getMember1() ? 1 : 2), member, true);
	}
	...
	public void onAppearObserverMode(Player observer)
	{
		...
		for(FightBattlePlayerObject member : getMembers())
		{
			...
			observer.sendPacket(new ExOlympiadUserInfo(player, member == getMember1() ? 1 : 2));
		}
	}
