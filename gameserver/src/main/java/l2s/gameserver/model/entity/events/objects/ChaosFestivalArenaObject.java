...
public final class ChaosFestivalArenaObject implements Serializable, Comparable<ChaosFestivalArenaObject>
{
	private class FinishTask extends RunnableImpl
	{
		...
		public void runImpl()
		{
			...
			switch(finishDelay)
			{
				...
				case 2:
				case 1:
					broadcastPacket(new SystemMessage2(SystemMsg.IN_S1_SECONDS_YOU_WILL_BE_MOVED_TO_WHERE_YOU_WERE_BEFORE_PARTICIPATING_IN_THE_CEREMONY_OF_CHAOS).addInteger(finishDelay));
					break;
				case 0:
					finishBattle();
					break;
	...
	private void showResultBoard()
	{
		...
		for(ChaosFestivalPlayerObject member : _members)
		{
			...
			if(_winner != null)
				player.sendPacket(new SystemMessage2(SystemMsg.CONGRATULATIONS_C1_YOU_WIN_THE_MATCH).addString(new CustomMessage("chaos_festival.player", player).add(_winner.getId()).toString()));
			else
