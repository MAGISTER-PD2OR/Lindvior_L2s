...
public class PartyVsPartyDuelEvent extends DuelEvent
{
	...
	public void askDuel(Player player, GameObject target)
	{
		...
		if(target.isPlayer())
		{
			...
			target.getPlayer().sendPacket(new SystemMessage2(SystemMsg.C1S_PARTY_HAS_CHALLENGED_YOUR_PARTY_TO_A_DUEL).addName(player), new ExDuelAskStart(player.getName(), 1));
		}
	}
