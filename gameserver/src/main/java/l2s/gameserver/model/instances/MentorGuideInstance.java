	...
	public void onBypassFeedback(Player player, String command)
	{
		...
		if(command.startsWith("changediplom"))
		{
			if(player.getInventory().getCountOf(33800) > 0)
			{
				...
				showChatWindow(player, "mentoring/menthelper.htm");
				return;
			}
			else
			{
				showChatWindow(player, "mentoring/menthelper-diplom-no.htm");
				return;
			}
		}
		else if(command.startsWith("takerewarddipl"))
		{
			if(player.getClassId().isOfLevel(ClassLevel.AWAKED))
			{
				MultiSellHolder.getInstance().SeparateAndSend(3358702, player, 0);
				return;
			}
			else
			{
				showChatWindow(player, "mentoring/menthelper-reward-no.htm");
				return;
			}
		}
		else
