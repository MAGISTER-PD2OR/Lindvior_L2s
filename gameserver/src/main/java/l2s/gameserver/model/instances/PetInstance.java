...
public class PetInstance extends Servitor
{
	...
	private void tryFeed()
	{
		...
		for(int foodId : getFoodId())
		{
			...
			if(food != null)
			{
				...
				{
					getPlayer().sendPacket(new SystemMessage2(SystemMsg.YOUR_PET_WAS_HUNGRY_SO_IT_ATE_S1).addItemName(food.getItemId()));
					if(Rnd.chance(5))
	...
	public void updateControlItem()
	{
		...
		player.sendPacket(new InventoryUpdate().addModifiedItem(player, controlItem));
	}
