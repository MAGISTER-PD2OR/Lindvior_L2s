...
import l2s.gameserver.network.l2.s2c.AcquireSkillDone;
...
public class NpcInstance extends Creature
{
	...
	public void teleportPlayer(Player player, int x, int y, int z, int itemId, long itemCount, int castleId, int reflectionId)
	{
		if(player == null)
			return;
	...
	public void teleportPlayer(Player player, Location loc, int itemId, long itemCount, int castleId, int reflectionId)
	{
		teleportPlayer(player, loc.getX(), loc.getY(), loc.getZ(), itemId, itemCount, castleId, reflectionId);
	}

	public void teleportPlayer(Player player, int x, int y, int z, int itemId, long itemCount)
	{
		teleportPlayer(player, x, y, z, itemId, itemCount, 0, -1);
	}

	public void teleportPlayer(Player player, Location loc, int itemId, long itemCount)
	{
		teleportPlayer(player, loc.getX(), loc.getY(), loc.getZ(), itemId, itemCount, 0, -1);
	}

	private void teleportPlayer(Player player, TeleportLocation loc, long itemCount)
	{
		teleportPlayer(player, loc, loc.getItemId(), loc.getPrice(), loc.getCastleId(), -1);
	}
		...
		if(list != null && !list.isEmpty() && player.getPlayerAccess().UseTeleport)
		{
			for(TeleportLocation tl : list.valueCollection())
			{
				if(tl.getItemId() == ItemTemplate.ITEM_ID_ADENA)
				{
					...
					sb.append("<a action=\"bypass -h npc_%objectId%_teleport_id_").append(listId).append("_").append(tl.getName()).append("_").append(price).append("\" msg=\"811;F;").append(tl.getName()).append("\">").append(HtmlUtils.htmlNpcString(tl.getName()));
					if(price > 0)
				...
				{
					sb.append("<a action=\"bypass -h npc_%objectId%_teleport_id_").append(listId).append("_").append(tl.getName()).append("\" msg=\"811;F;").append(tl.getName()).append("\">").append(HtmlUtils.htmlNpcString(tl.getName()));
	...
	{
		NpcHtmlMessage packet;
		if(filename.endsWith(".htm"))
			packet = new NpcHtmlMessage(player, this, filename, 0);
		else
		{
			packet = new NpcHtmlMessage(player, this);
