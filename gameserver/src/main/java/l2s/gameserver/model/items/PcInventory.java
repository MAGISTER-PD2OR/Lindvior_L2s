...
public class PcInventory extends Inventory
{
	...
	protected void sendAddItem(ItemInstance item)
	{
		...
		actor.sendPacket(new InventoryUpdate().addNewItem(actor, item));
	...
	protected void sendModifyItem(ItemInstance item)
	{
		...
		actor.sendPacket(new InventoryUpdate().addModifiedItem(actor, item));
		if(item.getItemId() == ItemTemplate.ITEM_ID_ADENA)
			actor.sendPacket(new ExAdenaInvenCount(actor));
		if(item.getTemplate().getAgathionEnergy() > 0)
			actor.sendPacket(new ExBR_AgathionEnergyInfo(1, item));
	...
	{
		Player actor = getActor();
		actor.sendPacket(new InventoryUpdate().addRemovedItem(actor, item));
		if(item.getItemId() == ItemTemplate.ITEM_ID_ADENA)
			actor.sendPacket(new ExAdenaInvenCount(actor));
	}
