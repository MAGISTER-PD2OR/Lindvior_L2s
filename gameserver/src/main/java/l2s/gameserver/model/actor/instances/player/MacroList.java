...
public class MacroList
{
	...
	public void registerMacro(Macro macro)
	{
		if(macro.id == 0)
		{
			...
			_owner.sendPacket(new SendMacroList(macro.id, SendMacroList.Action.ADD, 1, macro));
		}
		else
		{
			...
			_owner.sendPacket(new SendMacroList(macro.id, SendMacroList.Action.UPDATE, 1, macro));
		}
	}
	...
	public void deleteMacro(int id)
	{
		...
		_macroses.remove(id);
		_owner.sendPacket(new SendMacroList(id, SendMacroList.Action.DELETE, 0, null));
	}
	...
	public void sendMacroses()
	{
		...
		int size = size();
		if(size == 0)
			_owner.sendPacket(new SendMacroList(0, SendMacroList.Action.ADD, 0, null));
		else
		{
			for(Macro macro : _macroses.values())
				_owner.sendPacket(new SendMacroList(0, SendMacroList.Action.ADD, size, macro));
		}
	}
