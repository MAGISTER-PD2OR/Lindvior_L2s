...
public class Mount
{
	...
	public void onRide()
	{
		switch(getType())
		{
			case STRIDER:
				if(_rider.isNoble())
				{
					...
					_rider.sendPacket(new SkillList(_rider));
				}
				break;
			case WYVERN:
		...
		_rider.broadcastPacket(new Ride(_rider));
	...
	public void onUnride()
	{
		...
		_rider.broadcastPacket(new Ride(_rider));
		_rider.broadcastUserInfo(true); // нужно послать пакет после Ride для корректного отображения скорости
		_rider.sendPacket(new SetupGauge(_rider, SetupGauge.Colors.GREEN_MINI, 0));
	}
	...
	public void updateStatus()
	{
		...
		_rider.sendPacket(new SetupGauge(_rider, SetupGauge.Colors.GREEN_MINI, time, timeLost));
