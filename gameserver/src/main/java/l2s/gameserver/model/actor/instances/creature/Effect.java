...
public abstract class Effect extends RunnableImpl implements Comparable<Effect>, FuncOwner
{
	...
	public void addIcon(AbnormalStatusUpdate abnormalStatus)
	{
		if(!isActive() || isHidden())
			return;
		int duration = (_skill.isToggle() || _template.isHideTime()) ? AbnormalStatusUpdate.INFINITIVE_EFFECT : getTimeLeft();
		abnormalStatus.addEffect(getDisplayId(), getDisplayLevel(), duration);
	}

	public void addIcon(ExAbnormalStatusUpdateFromTarget abnormalStatus)
	{
		if(!isActive() || isHidden())
			return;
		int duration = (_skill.isToggle() || _template.isHideTime()) ? AbnormalStatusUpdate.INFINITIVE_EFFECT : getTimeLeft();
