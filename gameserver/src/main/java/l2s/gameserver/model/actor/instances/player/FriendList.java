...
public class FriendList
{
	...
	public void remove(String name)
	{
		...
		if(objectId > 0)
		{
			...
			if(friendChar != null)
				friendChar.sendPacket(new SystemMessage(SystemMessage.S1__HAS_BEEN_DELETED_FROM_YOUR_FRIENDS_LIST).addString(_owner.getName()), new L2FriendList(friendChar));
		}
		else
	...
	public IStaticPacket requestFriendInvite(GameObject target)
	{
		...
		if(contains(player.getObjectId()))
			return new SystemMessage2(SystemMsg.C1_IS_ALREADY_ON_YOUR_FRIEND_LIST).addName(player);
