...
public abstract class Servitor extends Playable
{
	...
	public void broadcastStatusUpdate()
	{
		...
		broadcastPacket(makeStatusUpdate(StatusUpdate.MAX_HP, StatusUpdate.CUR_HP));
	...
	public void displayGiveDamageMessage(Creature target, int damage, Servitor servitorTransferedDamage, int transferedDamage, boolean crit, boolean miss, boolean shld, boolean magic)
	{
		...
		if(miss)
			...
			getPlayer().sendPacket(new SystemMessage2(SystemMsg.C1_HAS_DONE_S3_POINTS_OF_DAMAGE_TO_C2).addName(this).addName(target).addInteger(damage).addHpChange(target.getObjectId(), getObjectId(), -damage));
	}
	...
	{
		if(attacker != this)
			getPlayer().sendPacket(new SystemMessage2(SystemMsg.C1_HAS_RECEIVED_S3_DAMAGE_FROM_C2).addName(this).addName(attacker).addInteger(damage).addHpChange(getObjectId(), attacker.getObjectId(), -damage));
	}
