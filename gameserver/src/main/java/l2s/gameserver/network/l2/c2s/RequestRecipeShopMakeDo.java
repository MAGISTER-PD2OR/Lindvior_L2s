...
public class RequestRecipeShopMakeDo extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if(Rnd.chance(recipe.getSuccessRate()))
		{
			...
			if(itemsCount > 1)
			{
				SystemMessage2 sm = new SystemMessage2(SystemMsg.C1_CREATED_S2_S3_AT_THE_PRICE_OF_S4_ADENA);
			...
			{
				SystemMessage2 sm = new SystemMessage2(SystemMsg.C1_CREATED_S2_AFTER_RECEIVING_S3_ADENA);
				sm.addName(manufacturer);
				sm.addItemName(itemId);
				sm.addLong(_price);
				buyer.sendPacket(sm);
		...
		{
			SystemMessage2 sm = new SystemMessage2(SystemMsg.C1_HAS_FAILED_TO_CREATE_S2_AT_THE_PRICE_OF_S3_ADENA);
			sm.addName(manufacturer);
			sm.addItemName(itemId);
			sm.addLong(_price);
			buyer.sendPacket(sm);
