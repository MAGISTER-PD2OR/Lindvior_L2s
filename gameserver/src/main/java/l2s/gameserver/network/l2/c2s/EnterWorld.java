...
import l2s.gameserver.network.l2.s2c.ExBR_PremiumState;
...
public class EnterWorld extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		activeChar.sendPacket(new ExBR_PremiumState(activeChar, activeChar.hasBonus()), new ExVitalityEffectInfo(activeChar));
		...
		sendPacket(new ExStorageMaxCount(activeChar), new QuestList(activeChar), new ExBasicActionList(activeChar), new EtcStatusUpdate(activeChar));
		...
		if(!first)
		{
			...
			if(servitors.length > 0)
			{
				for(Servitor servitor : servitors)
					sendPacket(new PetInfo(servitor));
			}
			...
			if(activeChar.isInParty())
			{
				...
				for(Player member : activeChar.getParty().getPartyMembers())
					if(member != activeChar)
					{
						...
						{
							for(Servitor servitor : servitors)
								sendPacket(new PartySpelled(servitor, true));
						}
		...
		if(!activeChar.getVarBoolean("@intro_god_video"))
		{
			...
			activeChar.sendPacket(UsmVideo.GD10_PROLOGUE.packet(activeChar));
		}
