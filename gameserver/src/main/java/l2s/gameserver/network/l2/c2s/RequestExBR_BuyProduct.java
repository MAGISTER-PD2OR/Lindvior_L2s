...
import l2s.gameserver.network.l2.s2c.ExBR_BuyProduct;
...
public class RequestExBR_BuyProduct extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if(product == null)
		{
			activeChar.sendPacket(ExBR_BuyProduct.RESULT_WRONG_PRODUCT);
			return;
		}

		if(!product.isOnSale() || (System.currentTimeMillis() < product.getStartTimeSale()) || (System.currentTimeMillis() > product.getEndTimeSale()))
		{
			activeChar.sendPacket(ExBR_BuyProduct.RESULT_SALE_PERIOD_ENDED);
			return;
		}
		...
		{
			activeChar.sendPacket(ExBR_BuyProduct.RESULT_NOT_ENOUGH_POINTS);
			return;
		}
		...
		{
			activeChar.sendPacket(ExBR_BuyProduct.RESULT_INVENTORY_FULL);
			return;
		}
		...
		activeChar.sendPacket(ExBR_BuyProduct.RESULT_OK);
