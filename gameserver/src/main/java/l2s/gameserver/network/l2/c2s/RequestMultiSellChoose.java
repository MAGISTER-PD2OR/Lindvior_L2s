		...
		try
		{
			...
			for(MultiSellIngredient ingridient : entry.getIngredients())
			{
				...
				if(ingridientItemId == ItemTemplate.ITEM_ID_CLAN_REPUTATION_SCORE)
				{
					...
					{
						activeChar.sendPacket(new SystemMessage2(SystemMsg.S1_IS_NOT_A_CLAN_LEADER).addName(activeChar));
						return;
					}
			...
			for(ItemData id : items)
			{
				...
				if(count > 0)
					if(id.getId() == ItemTemplate.ITEM_ID_CLAN_REPUTATION_SCORE)
					{
						...
						activeChar.sendPacket(new SystemMessage2(SystemMsg.S1_POINTS_HAVE_BEEN_DEDUCTED_FROM_THE_CLANS_REPUTATION).addLong(count));
					}
					...
					else if(id.getId() == ItemTemplate.ITEM_ID_FAME)
					{
						...
						activeChar.sendPacket(new SystemMessage2(SystemMsg.S2_S1_HAS_DISAPPEARED).addLong(count).addString("Fame"));
					}
					else
					{
