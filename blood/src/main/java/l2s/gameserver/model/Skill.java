	...
	public final Creature getAimingTarget(Creature activeChar, GameObject obj)
	{
		...
		switch(_targetType)
		{
			...
			case TARGET_ELEMENTAL_DES:
				/*System.out.println("target.getEffectList().containsEffects(11101) " + target.getEffectList().containsEffects(11101));
				System.out.println("target.getEffectList().containsEffects(11102) " + target.getEffectList().containsEffects(11102));
				System.out.println("target.getEffectList().containsEffects(11103) " + target.getEffectList().containsEffects(11103));
				System.out.println("target.getEffectList().containsEffects(11104) " + target.getEffectList().containsEffects(11104));
				System.out.println("target.getEffectList().containsEffects(11105) " + target.getEffectList().containsEffects(11105));
				*/
				target = (target != null && (
...
//				return target != null && target.isDead() == _isCorpse && !(target == activeChar && isOffensive()) && (!_isUndeadOnly || target.isUndead()) && activeChar.getPlayer().isInSameClan(target.getPlayer()) ? target : null;
	...
	public final void getEffects(final Creature effector, final Creature effected, final boolean self, final long timeConst, final double timeMult, boolean reflectedSkill)
	{
		...
		for(Creature target : targets)
		{
			...
			if(scheduledCount > 0 && !isHideStartMessage())
				target.sendPacket(new SystemMessage2(SystemMsg.S1S_EFFECT_CAN_BE_FELT).addSkillName(getDisplayId(), getDisplayLevel()));

			if(target == effected)
			{
				if(resistedDebuff || resistedBuff)
					effector.sendPacket(new SystemMessage2(SystemMsg.C1_HAS_RESISTED_YOUR_S2).addName(target).addSkillName(_displayId, _displayLevel));
	...
	public void useSkill(Creature activeChar, List<Creature> targets)
	{
		if(isVitalityLimited() && activeChar.isPlayer())
		{
			...
			player.sendPacket(new SystemMessage2(SystemMsg.VITALITY_EFFECT_IS_APPLIED_THERES_S1_VITALITY_EFFECT_LEFT_THAT_MAY_BE_APPLIED_UNTIL_THE_NEXT_CYCLE).addInteger(player.getVitalityPotionsLeft()));
		}
