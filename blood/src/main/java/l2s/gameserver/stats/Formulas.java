...
public class Formulas
{
	...
	public static AttackInfo calcPhysDam(Creature attacker, Creature target, Skill skill, boolean dual, boolean blow, boolean ss, boolean onCrit)
	{
		...
		if(skill != null)
		{
			...
			if(info.blow)
			{
				info.damage *= 0.01 * attacker.calcStat(Stats.P_CRITICAL_DAMAGE, target, skill);
			...
			if(skill.isChargeBoost())
			{
				...
				if(force > 3)
					force = 3;
				if(skill.getId() == 10260 && attacker.getIncreasedForce() == 0) //for now hardcoded?	glory chance no penalty if no forces used.		
					info.damage *= 1 + 0.2 * force;
				else
					info.damage *= 0.8 + 0.2 * force;
			}
		...
		else
		{
			...
			if(info.crit)
			{
				info.damage *= 0.01 * attacker.calcStat(Stats.P_CRITICAL_DAMAGE, target, null);
	...
	public static AttackInfo calcMagicDam(Creature attacker, Creature target, Skill skill, int sps)
	{
		...
		if(info.crit)
		{
			...
			info.damage = info.damage * getMCritDamageMode(attacker);
		}
		else
		...
		if(Rnd.chance(failChance))
		{
			...
			{
				info.damage /= 2;
				SystemMessage2 msg = new SystemMessage2(SystemMsg.DAMAGE_IS_DECREASED_BECAUSE_C1_RESISTED_C2S_MAGIC).addName(target).addName(attacker);
				attacker.sendPacket(msg);
				target.sendPacket(msg);
			}
		}
	...
	public static boolean calcPCrit(Creature attacker, Creature target, Skill skill, boolean blow)
	{
		...
		if(skill != null)
		{
			double statModifier = blow || (attacker.calcStat(Stats.P_SKILL_CRIT_RATE_DEX_DEPENDENCE) > 0) ? BaseStats.DEX.calcBonus(attacker) : BaseStats.STR.calcBonus(attacker);
	...
	public static boolean calcHitMiss(Creature attacker, Creature target)
	{
		...
		chanceToHit = Math.min(chanceToHit, 98.0);
	...
	private static boolean calcMagicHitMiss(Creature attacker, Creature target)
	{
		...
		chanceToHit = Math.max(chanceToHit, 72.5);
		chanceToHit = Math.min(chanceToHit, 98.0);
	...
	public static boolean calcSkillSuccess(Env env, EffectTemplate et, int spiritshot)
	{
		...
		if(skill.isCancelSkill() && target.isPlayer() && target.getPlayer().getClassId().isOfLevel(ClassLevel.AWAKED))
		...
		{
			if(target.isPlayer() && caster.isPlayer() && target.getPlayer().getClassId().isOfLevel(ClassLevel.AWAKED) && !caster.getPlayer().getClassId().isOfLevel(ClassLevel.AWAKED))
			{
				if(Config.NEW_CHANCE_FOR_NOT_REBORNED_SKILLS > 0)
					return Rnd.chance(Config.NEW_CHANCE_FOR_NOT_REBORNED_SKILLS);
				return false;
			}
			if(target.isPlayer() && caster.isPlayer() && caster.getPlayer().getClassId().isOfLevel(ClassLevel.AWAKED) && target.getPlayer().getClassId().isOfLevel(ClassLevel.AWAKED) && checkIfNotRebornSkill(caster.getPlayer(), skill))
			{
				if(Config.NEW_CHANCE_FOR_NOT_REBORNED_SKILLS > 0)
					return Rnd.chance(Config.NEW_CHANCE_FOR_NOT_REBORNED_SKILLS);
				return false;
	...
	public static void calcSkillMastery(Skill skill, Creature activeChar)
	{
		...
		if(activateINT)
		{
			double chanceINT = activeChar.calcStat(Stats.SKILL_MASTERY, activeChar.getINT() / 2) / 10; // TODO: [Bonux] Уточнить формулу.
		...
		if(!calcSkillMastery)
		{
			...
			if(activateSTR)
			{
				double chanceSTR = activeChar.calcStat(Stats.SKILL_MASTERY, activeChar.getSTR() / 2) / 10; // TODO: [Bonux] Уточнить формулу.
...
//DEPRECATED
	...
	public static int calculateKarmaLost(Player player, long exp)
	{
		...
		if(exp > 0) // Received exp
			exp /= Config.KARMA_RATE_KARMA_LOST == -1 ? Config.RATE_XP_BY_LVL[player.getBaseSubClass().getLevel()] : Config.KARMA_RATE_KARMA_LOST;
		return (int) ((Math.abs(exp) / karmaLooseMul) / 1500);
