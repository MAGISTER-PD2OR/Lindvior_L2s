...
public class RequestMenteeAdd extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if(!activeChar.getClassId().isOfLevel(ClassLevel.AWAKED))
		{
		...
		if(newMentee == null) // Чар онлайн?
		{
			...
			{
				// Выше 85 лвла
				if(fakePlayer.getLevel() > 84)
				{
					activeChar.sendPacket(new SystemMessage2(SystemMsg.S1_IS_ABOVE_LEVEL_86_AND_CANNOT_BECOME_A_MENTEE).addName(fakePlayer));
					return;
				}

				new Request(L2RequestType.MENTEE, activeChar, null).setTimeout(10000L);
				activeChar.sendPacket(new SystemMessage2(SystemMsg.YOU_HAVE_OFFERED_TO_BECOME_S1_MENTOR).addName(fakePlayer));
				return;
			}
